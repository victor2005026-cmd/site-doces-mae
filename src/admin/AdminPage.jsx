...
export default function AdminPage() {
  const { user, perfil, isAdmin, loading } = useAuth();
  ...
  // usuário já logado, mas o perfil (que traz eh_admin) ainda não chegou
  if (!perfil) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-bg-alt">
  ...
  if (!isAdmin) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-bg-alt">
        ...
        <Link to="/" className="text-rose underline">Voltar ao site</Link>
      </div>
    );
  }
