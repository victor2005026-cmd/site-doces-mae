...
Deno.serve(async (req) => {
  ...
  // Mesma trava contra duplicidade do aviso de pedido novo, só que numa
  // coluna própria — os dois eventos são independentes.
  const agora = new Date().toISOString();
  const { data: pedido, error: pedidoError } = await admin
    .from('pedidos')
    .update({ notificacao_pagamento_whatsapp_enviada_em: agora })
    .eq('id', pedidoId)
    .is('notificacao_pagamento_whatsapp_enviada_em', null)
    .select('numero_pedido, total, dados_convidado, perfis(nome, telefone)')
    .maybeSingle();
  ...
  try {
    ...
    await admin.from('pedidos').update({ notificacao_pagamento_whatsapp_enviada_em: null }).eq('id', pedidoId);
